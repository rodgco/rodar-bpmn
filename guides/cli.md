...
Starts the application, parses the BPMN file, and runs the first process. The task automatically attempts convention-based handler auto-discovery, looking for handler modules at the paths generated by `mix rodar_bpmn.scaffold` (e.g., `MyApp.Bpmn.OrderProcessing.Handlers.ValidateOrder`). Any discovered handlers are wired automatically; tasks without handlers use a passthrough that lets execution continue.

The header output shows which handlers were discovered and which are using passthrough:

```
  Discovered handlers:
    - Task_Validate → MyApp.Bpmn.OrderProcessing.Handlers.ValidateOrder
...
- **Output directory**: `lib/<app_name>/bpmn/<bpmn_filename>/handlers/`
...
Scaffolded handlers are placed at deterministic module paths following the convention `<AppName>.Bpmn.<BpmnFilename>.Handlers.<TaskName>`. This means after scaffolding, you can use auto-discovery instead of manual wiring:
...
# All scaffolded handlers at MyApp.Bpmn.OrderProcessing.Handlers.* are found automatically
```
