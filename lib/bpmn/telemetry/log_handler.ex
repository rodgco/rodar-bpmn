defmodule Bpmn.Telemetry.LogHandler do
  @moduledoc """
  ...
  ## Usage
      ...
      Bpmn.Telemetry.LogHandler.detach()

  """

  require Logger

  @handler_id "bpmn-default-log-handler"
  ...
  @doc false
  def handle_event([:bpmn, :node, :start], _measurements, metadata, _config) do
    Logger.debug(fn ->
      "BPMN node started: #{metadata.node_id} (#{metadata.node_type}) token=#{metadata.token_id}"
    end)
  end

  def handle_event([:bpmn, :node, :stop], measurements, metadata, _config) do
  ...
  def handle_event([:bpmn, :node, :exception], measurements, metadata, _config) do
    duration_us = System.convert_time_unit(measurements.duration, :native, :microsecond)
  ...
  def handle_event([:bpmn, :process, :start], _measurements, metadata, _config) do
    Logger.info(fn ->
      "BPMN process started: instance=#{metadata.instance_id} process=#{metadata.process_id}"
    end)
  end

  def handle_event([:bpmn, :process, :stop], measurements, metadata, _config) do
    duration_us = System.convert_time_unit(measurements.duration, :native, :microsecond)
  ...
  def handle_event([:bpmn, :token, :create], _measurements, metadata, _config) do
    Logger.debug(fn ->
  ...
  def handle_event([:bpmn, :event_bus, :publish], _measurements, metadata, _config) do
    Logger.debug(fn ->
  ...
  def handle_event([:bpmn, :event_bus, :subscribe], _measurements, metadata, _config) do
    Logger.debug(fn ->
